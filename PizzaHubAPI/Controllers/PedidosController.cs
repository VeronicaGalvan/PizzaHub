...
#pragma warning disable CS8602 // Disable possible null dereference warnings in controller mappings (handled defensively)
...
public class PedidosController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<PedidoResumenDTO>>> GetPedidos()
    {
        ...
        var query = _context.Pedidos
            .Include(p => p.Cliente).ThenInclude(c => c.Persona!)
            .Include(p => p.Repartidor).ThenInclude(r => r.Persona!)
        ...
        if (!esAdmin)
        {
            if (esRepartidor)
            {
                query = query.Where(p => p.Repartidor != null && p.Repartidor.Persona != null && p.Repartidor.Persona.UsuarioId == usuarioId);
            }
            else
            {
                query = query.Where(p => p.Cliente != null && p.Cliente.Persona != null && p.Cliente.Persona.UsuarioId == usuarioId);
            }
        }
        ...
        var pedidos = await query
            ...
            .Select(p => new PedidoResumenDTO
            {
                ...
                CreadoEn = p.CreadoEn,
                Cliente = (p.Cliente != null && p.Cliente.Persona != null) ? $"{p.Cliente.Persona.Nombre} {p.Cliente.Persona.Apellido}" : null,
                Repartidor = (p.Repartidor != null && p.Repartidor.Persona != null) ? $"{p.Repartidor.Persona.Nombre} {p.Repartidor.Persona.Apellido}" : null,
    ...
    public async Task<ActionResult<PedidoResumenDTO>> GetPedido(int id)
    {
        ...
        // Verificar permisos (proteger accesos si Cliente/Repartidor o su Persona son nulos)
        var pedidoClienteUserId = pedido.Cliente != null && pedido.Cliente.Persona != null ? pedido.Cliente.Persona.UsuarioId : (int?)null;
        var pedidoRepartidorUserId = pedido.Repartidor != null && pedido.Repartidor.Persona != null ? pedido.Repartidor.Persona.UsuarioId : (int?)null;

        if (!esAdmin && (pedidoClienteUserId == null || pedidoClienteUserId != usuarioId) && (!esRepartidor || (pedidoRepartidorUserId == null || pedidoRepartidorUserId != usuarioId)))
        {
            return Forbid();
        }
        ...
        return new PedidoResumenDTO
        {
            ...
            Cliente = (pedido.Cliente != null && pedido.Cliente.Persona != null) ? $"{pedido.Cliente.Persona.Nombre} {pedido.Cliente.Persona.Apellido}" : null,
            Repartidor = (pedido.Repartidor != null && pedido.Repartidor.Persona != null) ? $"{pedido.Repartidor.Persona.Nombre} {pedido.Repartidor.Persona.Apellido}" : null,
    ...
    public async Task<ActionResult<PedidoResumenDTO>> CreatePedido(CrearPedidoDTO pedidoDto)
    {
        ...
        var clienteEntity = await _context.Clientes.Include(c => c.Persona)
            .FirstOrDefaultAsync(c => c.Persona.UsuarioId == usuarioId);
        if (clienteEntity == null)
            return BadRequest("Cliente no encontrado para el usuario");
